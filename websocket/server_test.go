...
func TestGame(t *testing.T) {
	...
	t.Run("start a game with 3 players, send some blind alerts down WS and declare Ruth the winner", func(t *testing.T) {
		...
		winner := "Ruth"
		msgWaitTime := 10 * time.Millisecond
		...
		time.Sleep(msgWaitTime)

		assertGameStartedWith(t, game, 3)
		assertFinishCalledWith(t, game, winner)
		within(t, msgWaitTime, func() { assertWebsocketGotMsg(t, ws, wantedBlindAlert) })
	})
}
...
func mustMakePlayerServer(t *testing.T, store poker.PlayerStore, gameSpy *GameSpy) *poker.PlayerServer {
